...
bool Lobby::crearPartida(const PartidaDTO& partida_a_crear, HandlerCliente *handler) {
    ...
    if (busqueda == this->partidas_creadas.end()) {
        ...
        partida_nueva.jugadores.push_back(handler);
...
bool Lobby::unirAPartida(const PartidaDTO& partida_a_unirse, HandlerCliente *handler) {
    std::lock_guard<std::mutex> lock(m);
    ...
    if (partida == this->partidas_creadas.end()) {
        ...
        if (partida->second.jugadores_actuales < partida->second.jugadores_requeridos) {
            partida->second.jugadores.push_back(handler);
