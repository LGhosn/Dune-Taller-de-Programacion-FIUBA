...
PalacioSDL::PalacioSDL(uint8_t id, uint8_t id_jugador, SDL2pp::Renderer& renderer,
                        ...
                        EdificioSDL(id, id_jugador, renderer, texturas.obtenerEdificio(4, casa, false),
...
void PalacioSDL::cambiarHP(uint16_t hp_edificio) {
    if (hp_edificio < limite_hp_debilitar)
        debilitado = true;
}

void PalacioSDL::update(uint32_t origen_movil_x, uint32_t origen_movil_y, long frame_actual, float zoom) {
    this->zoom = zoom;
